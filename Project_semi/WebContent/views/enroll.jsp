...
<body>
	...
	<div id="container">
		...
		<div id="content">
				...
				<form action="/enroll" method="post">
					<div class="form-group">
						<label for="memberId">ID</label> <input type="text"
							class="form-control text" id="memberId" name="memberId"
							onkeyup="return IdCheck()"> <span id="test1"></span>
					</div>

					<div id="messageOne"></div>
					<br>
					<div class="form-group">
						<label for="memberPwd">PW</label> <input type="password"
							class="form-control text" id="memberPwd" name="memberPwd"
							onkeyup="return PwdCheck()">
					...
					<div class="form-group">
						<label for="memberPwd_re">PW확인</label> <input type="password"
							class="form-control text" id="memberPwd_re" name="memberPwd_re"
							onkeyup="return Pwd_reCheck()">
					</div>

					<div id="messageThree"></div>
					<div class="form-group">
					...
					<div class="form-group" id="emailDiv">
						...
						<input type="text" class="form-control text" id="email" name="email"
							onkeyup="return emailCheck()"> <span id="span1">@</span>
						<input type="text" class="form-control text" id="email2"
							name="email2" onkeyup="return email2Check()">
					</div>
					<div id="messageSix"></div>
			...
			<div id="content3">
				...
				<button class="btn btn-primary" id="check-btn">중복검사</button>
			</div>
		</div>
	...
	<script>
		window.onload = function() {
			message1 = document.getElementById("messageOne");
			message2 = document.getElementById("messageTwo");
			message3 = document.getElementById("messageThree");
			message4 = document.getElementById("messageFour");
			message5 = document.getElementById("messageFive");
			message6 = document.getElementById("messageSix");
		...
		function IdCheck() {
			/* init(); */
			if (!(/^[a-z][a-z0-9_-]{4,11}$/.test(memberId.value))) //아이디 검사
			{
				memberId.style.border = "1px solid red";
				message1.innerHTML = "아이디는 소문자,숫자만 가능하고 4~12글자 입력바랍니다.";
				return false;
			} else {
				message1.innerHTML = "";
		...
		function PwdCheck() {
			/* 	init(); */
			if (!(/[a-zA-z0-9]{8,50}$/.test(memberPwd.value))) {
				memberPwd.style.border = "1px solid red";
				message2.innerHTML = "비밀번호는 소문자, 대문자, 숫자 중 8~12글자로 입력 바랍니다.";
				return false;
			} else {
				message2.innerHTML = "";
		...
		function Pwd_reCheck() {
			/* init(); */
			if ((memberPwd.value != memberPwd_re.value)) {
				memberPwd_re.style.border = "1px solid red";
				message3.innerHTML = "비밀번호와 비밀번호 확인 값은 같아야 합니다.";
				return false;
			} else {
		...
		function emailCheck() {
			if ($("#email").val() == "") {
				...
				message6.innerHTML = "이메일을 입력해주세요.";
				return false;
			} else {
		...
		function check() {
			if (memberId.value == "" && memberId.value.length == 0) {
				memberId.style.border = "1px solid red";
				message1.innerHTML = "아이디를 입력하세요";
				return false;
			} else if (memberPwd.value == "" && memberPwd.value.length == 0) {
				memberPwd.style.border = "1px solid red";
				message2.innerHTML = "비밀번호를 입력하세요";
				return false;
			} else if (memberPwd_re.value == ""
					&& memberPwd_re.value.length == 0) {
				memberPwd_re.style.border = "1px solid red";
				return false;
			} else if (memberName.value == "" && memberName.value.length == 0) {
				memberName.style.border = "1px solid red";
				...
				return false;
					...
					&& email2.value == "" && email2.value.length == 0) {
				email.style.border = "1px solid red";
				email2.style.border = "1px solid red";
				message6.innerHTML = "이메일를 입력하세요";
				return false;
